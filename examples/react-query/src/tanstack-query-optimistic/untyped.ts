...
useMutation;
import { InjectionPrimitives, stopInjection } from "./decorate";
import {
...
export function _attachOptimisticFunctionality(client: QueryClient) {
  const inject = new InjectionPrimitives(client);
  return {
    optimisticData<D extends AnyDef>(
      config: UntypedConfigs<D>["optimisticData"]
    ) {
      return inject.watchMutationEvents<D["source"]["input"]>(
        config.from,
        () => {
          let queryInjection:
            | undefined
            | ReturnType<InjectionPrimitives["injectQueryData"]>;
          ...
          return {
            ...
            onChange(event) {
              if (event.status !== "idle") {
                queryInjection ??= inject.injectQueryData<
                  D["target"]["output"]
                >(
                  config.to(event.variables),
                  config.emptyDefaultIfMutationBeforeQuery,
                  () => {
